...
Peso de peixes (em quilos): <input type="text" name="peso"><br><br>
<input type="submit">
</form>

<?php
function calcularExcesso($peso) {
    $limite = 50;
    $excesso = $peso - $limite;
    return $excesso > 0? $excesso : 0;
}

function calcularMulta($excesso) {
    $multaPorQuilo = 4.00;
    return $excesso * $multaPorQuilo;
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $peso = $_POST["peso"];
    $excesso = calcularExcesso($peso);
    $multa = calcularMulta($excesso);
    
    echo "Excesso de peso: ". $excesso. " quilos<br>";
    echo "Multa a ser paga: R$ ". $multa. "<br>";
}
?>